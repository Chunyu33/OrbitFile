// 用户手册组件
// 说明所有功能的原理，尤其是软链接机制与 Windows 自带"移动文件夹"的区别
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-5">
      <h3 className="text-[13px] font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>{title}</h3>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="用户手册" width={680}>
      ...
      <Section title="概述">
        ...
        <DocP>
          这种方式的核心优势是：<strong>应用无需重新安装，功能完全不受影响</strong>，用户无感知。
        </DocP>
      </Section>

      {/* 核心原理 */}
      <Section title="核心原理：NTFS 目录联结（Junction）">
        <DocP>
          <strong>目录联结（Junction）</strong> 是 Windows NTFS 文件系统原生支持的符号链接类型。
          它在文件系统层面创建一个 "指针"：当任何程序（包括 Windows 资源管理器）访问原始路径时，
          系统自动将请求重定向到目标位置。
        </DocP>
        <DocP>
          迁移流程：
        ...
        <ol className="list-decimal pl-5 mb-2 space-y-1">
          <li>将原始文件夹中的所有文件复制到用户选择的目标位置</li>
          <li>验证复制完整性（确保无数据丢失）</li>
          <li>将原始文件夹重命名为备份</li>
          <li>在原始位置创建一个 Junction，指向目标位置</li>
      ...
      {/* 与 Windows 自带功能的区别 — 关键说明 */}
      <Section title="与 Windows 右键 → 属性 → 位置 → 移动的区别（重要）">
        <DocP>
          Windows 资源管理器为"桌面""文档""图片"等系统文件夹提供了
          <strong>"属性 → 位置 → 移动"</strong> 功能。这与 OrbitFile 的 Junction 机制有本质区别：
        </DocP>
        ...
        <div className="rounded p-3 mb-3 text-[11px] leading-relaxed"
          ...
          <strong style={{ color: 'var(--color-warning)' }}>Windows "移动文件夹" 的问题：</strong>
          <ul className="list-disc pl-4 mt-1 space-y-0.5" style={{ color: 'var(--text-secondary)' }}>
            ...
            <li>移动过程中如果中断，可能导致文件夹位置不一致</li>
          </ul>
        </div>

        <div className="rounded p-3 mb-3 text-[11px] leading-relaxed"
          style={{ background: 'var(--color-primary-light)', border: '1px solid var(--color-primary)' }}>
          <strong style={{ color: 'var(--color-primary)' }}>OrbitFile 的 Junction 优势：</strong>
          <ul className="list-disc pl-4 mt-1 space-y-0.5" style={{ color: 'var(--text-secondary)' }}>
        ...
        <DocP>
          <strong>一句话总结：</strong>Windows 自带功能是"告诉 Windows 文件夹换了个地方"，
          而 OrbitFile 是"在文件系统层面做了一个透明的跳转"。后者更底层、更通用。
        </DocP>
      </Section>
      ...
      <Section title="功能说明">
        ...
        <DocP>
          <strong>迁移历史：</strong>记录所有迁移操作，支持查看详情、检查目标位置是否可用、
          一键恢复迁移。如果目标磁盘被移除或路径损坏，会显示异常状态。
        </DocP>
        <DocP>
          <strong>应用卸载（强力卸载）：</strong>运行应用自带的卸载程序，卸载完成后自动扫描残留文件和注册表项，
          提供清理选项。相比 Windows 自带卸载，能更彻底地清除残留。
        </DocP>
        <DocP>
      ...
      <Section title="强力卸载（详解）">
        ...
        <ol className="list-decimal pl-5 mb-2 space-y-1">
          ...
          <li>
            <strong>残留扫描：</strong>卸载完成后，自动扫描以下位置是否有残留文件和注册表项：
          ...
          <li>
            <strong>清理确认：</strong>将扫描到的残留列出，默认全选，用户确认后删除（支持回收站或彻底删除两种模式）。
          </li>
        </ol>
        ...
        <div className="rounded p-3 mb-3 text-[11px] leading-relaxed"
          ...
          <ul className="list-disc pl-4 mt-1 space-y-0.5" style={{ color: 'var(--text-secondary)' }}>
            ...
            <li>对于没有卸载程序的应用（便携软件等），可通过文件管理器手动删除安装目录</li>
          </ul>
        </div>
      ...
      {/* 数据安全 */}
      <Section title="数据安全说明">
        <DocP>
          迁移过程遵循 <strong>"先复制、后验证、再替换"</strong> 的安全流程：
        </DocP>
        <ul className="list-disc pl-5 mb-2 space-y-0.5">
          <li>迁移前：检测是否有程序正在占用文件夹，防止迁移过程中文件被修改</li>
          <li>复制阶段：完整复制所有文件到目标位置，保留目录结构</li>
          <li>验证阶段：确认目标位置文件完整且可访问</li>
          <li>替换阶段：将原始文件夹重命名为备份后才创建 Junction</li>
          <li>回滚能力：如果在任何阶段出错，可从备份恢复</li>
        </ul>
        <DocP>
